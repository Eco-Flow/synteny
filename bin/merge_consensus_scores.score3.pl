...
foreach my $key ( keys %pairs ) {
	...
	my $Junc_inter=($line_run1[19]+$line_run2[19])/2;
	print $out "$Junc_inter\t";
	my $Junc_inver=($line_run1[20]+$line_run2[20])/2;
	print $out "$Junc_inver\t";
	my $Junc_indel=($line_run1[21]+$line_run2[21])/2;
	print $out "$Junc_indel\t";
	my $Junc_indel_lt_5=($line_run1[22]+$line_run2[22])/2;
	print $out "$Junc_indel_lt_5\t";
	my $Junc_indel_5_20=($line_run1[23]+$line_run2[23])/2;
	print $out "$Junc_indel_5_20\t";
	my $Junc_indel_gt_20=($line_run1[24]+$line_run2[24])/2;
	print $out "$Junc_indel_gt_20\n";
}
