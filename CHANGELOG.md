...
- Optional `--tree_calibrations` flag for the `ALGO` subworkflow: time-calibrates whichever species tree is in use (built by `--iqtree_species_tree`, or supplied via `--species_tree`) with `ape::chronos`, converting branch lengths from substitutions/site to millions of years, via a new `DATE_TREE` module (`bin/date_tree.R`, `modules/local/algo/date_tree.nf`) ported from [Eco-Flow/excon (tree_subsampling branch)](https://github.com/Eco-Flow/excon/tree/tree_subsampling) (`bin/date_tree.R`, itself adapted there from `make_dated_trees.R` by Tom Wenseleers, KU Leuven 2026 -- provenance kept in this pipeline's copy)
  - New params: `tree_calibrations`, `chronos_model` (default `discrete`), `chronos_lambda` (default `1`), `chronos_rate_categories` (default `10`), matching excon's own names/defaults
  - Uses `quay.io/ecoflowucl/cafe:r-4.3.1` (R + `ape`), fully registry-qualified to match every other module's container in this pipeline -- excon's own module relies on a pipeline-wide `docker.registry = "quay.io"` default this pipeline doesn't set, which would otherwise resolve to the wrong (nonexistent) Docker Hub image; confirmed by reproducing the pull failure before fixing it
  - Off by default; a tree supplied via `--species_tree` that is already time-calibrated needs no separate flag -- it is simply used as-is when `--tree_calibrations` is unset, same as today
...
- Related but separate bug found on the same real run, one step later: `syngraph infer`'s own tree loader (ete3, inside the container, not something this pipeline can patch) rejects IQ-TREE's combined SH-aLRT/UFBoot support format (e.g. `100/100`) outright with a `NewickError`, since it expects a single plain numeric support value per internal node. New `STRIP_TREE_SUPPORT` module (`bin/strip_tree_support.py`, `modules/local/algo/strip_tree_support.nf`) strips internal-node support labels (keeping branch lengths and leaf names) before the tree reaches `SYNGRAPH` and `SYNGRAPH_BOOTSTRAP` -- Syngraph doesn't use support values for anything, only topology and branch lengths, so nothing is lost. `AGORA_PREP` and the published `species_tree/SpeciesTree_rooted.nwk` are untouched, so support values remain visible there. Runs unconditionally (not gated on `--iqtree_species_tree`), since a user-supplied `--species_tree` carrying support values from RAxML/other tools would hit the same crash
